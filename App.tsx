...
const App: React.FC = () => {
  ...
  const deduplicateByField = <T,>(data: T[], field: string): T[] => {
  ...
  const fetchRealData = useCallback(async () => {
    ...
    try {
      ...
      if (isMounted) {
        if (guardsRes.data) setGuards(deduplicateByField(guardsRes.data, 'id'));
        if (sitesRes.data) setSites(deduplicateByField(sitesRes.data as Site[], 'id'));
        if (profilesRes.data) setProfiles(deduplicateByField(profilesRes.data as Profile[], 'id'));
        if (companiesRes.data) setCompanies(deduplicateByField(companiesRes.data as Company[], 'id'));
        if (codesRes.data) setDisciplinaryCodes(deduplicateByField(codesRes.data as DisciplinaryCode[], 'code'));
        if (recordsRes.data) {
          const rawRecords = recordsRes.data as DisciplinaryRecord[];
          setDisciplinaryRecords(deduplicateByField(rawRecords, 'id'));
          ...
          setIncidents(deduplicateByField(mappedIncidents, 'id'));
        }
        setLoading(false);
      }
    } catch (error) {
  ...
  useEffect(() => {
    const refreshOnBlacklist = async () => {
      ...
      try {
        ...
        if (res?.data) {
          setBlacklistedGuards(deduplicateByField(res.data as Guard[], 'id'));
        } else {
  ...
  const handlePublicApply = (newGuard: Guard) => {
    ...
    setGuards(prev => deduplicateByField([...prev, newGuard], 'id'));
  ...
  const handleIntakeComplete = async (newGuard: Guard, isApplicantFlow = false) => {
    if (isApplicantFlow) {
      try {
        ...
        if (data && data.id) {
          setGuards(prev => deduplicateByField(prev.map(g => (g?.id === data.id) ? { ...g, ...data } : g) as Guard[], 'id'));
      ...
      try {
        ...
        const data = result.data as Guard | undefined;
        if (data) {
          setGuards(prev => deduplicateByField([...prev, data], 'id'));
          (window as any).showNotification?.('success', 'Applicant registered and interview locked.');
        } else {
          setGuards(prev => deduplicateByField([...prev, { ...payload, id: `g-${Date.now()}` } as Guard], 'id'));
          (window as any).showNotification?.('warning', 'Offline: personnel saved locally.');
        }
      } catch (e) {
        setGuards(prev => deduplicateByField([...prev, { ...newGuard, company_id: userCompanyId, id: `g-${Date.now()}` } as Guard], 'id'));
            ...
            {activeTab === 'sites' && (userRole === UserRole.COMPANY_ADMIN || userRole === UserRole.SUPER_ADMIN) && (
              <SiteManager
                ...
                onAddSite={async (site) => {
                  const newSite = {
                    ...site,
                    company_id: (site.company_id || userCompanyId) as string
                  } as Site;
                  try {
