...
import { Guard, ApplicationStatus, Site, DisciplinaryCode, IncidentReport } from '../types';
...
const OperationsEngine: React.FC<OperationsEngineProps> = ({ 
  ...
  const activeGuards = useMemo(() => guards.filter(g => g.application_status === ApplicationStatus.ACTIVE), [guards]);
  ...
  const handleSubmit = async () => {
    if (!selectedGuardId || !incidentCode || !incidentNotes) {
           ...
           <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-white/20">
              ...
              <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
                 ...
                 <div className="space-y-2">
                     ...
                     <FileUploader label="Upload Photo/Doc" fileUrl={evidence} onUpload={setEvidence} onRemove={() => setEvidence('')} className="!h-24" />
                 </div>
              </div>
