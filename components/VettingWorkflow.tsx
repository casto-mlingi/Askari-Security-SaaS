              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {sortedPoolApplicants.length > 0 ? sortedPoolApplicants.map(guard => (
                  <div
                    ...
                    className="group relative flex flex-col bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-500 cursor-pointer overflow-hidden"
                  >
                    ...
                    <div className="relative flex justify-between items-start mb-8">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <h4 className="font-black text-slate-900 uppercase tracking-tight text-xl leading-tight group-hover:text-primary transition-colors">
                            {guard.full_name}
                      ...
                      <div className="relative w-16 h-16 flex items-center justify-center shrink-0">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center leading-none">
                          <span className="text-lg font-black text-slate-900 font-hud">{guard.readiness_score || 0}</span>
                          <span className="text-[6px] font-black text-slate-400 uppercase tracking-tighter">Readiness</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-8">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex flex-col items-center">
                        <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Age</span>
                        <span className="text-xs font-black text-slate-700">{safeAge(guard.dob).replace(' Years', '')}</span>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex flex-col items-center">
                        <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Edu</span>
                        <span className="text-[10px] font-black text-slate-700 truncate w-full text-center">{guard.education_history[0]?.level?.slice(0, 7) || 'None'}</span>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex flex-col items-center">
                        <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Status</span>
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                      {[
                        ...
                        <span key={idx} className="px-2 py-1 bg-slate-50 text-[7px] font-black text-slate-500 uppercase tracking-widest rounded-lg border border-slate-100">
                          {d.label} ✓
                        </span>
                      ))}
                      <span className="px-2 py-1 bg-slate-50 text-[7px] font-black text-slate-500 uppercase tracking-widest rounded-lg border border-slate-100">
                    ...
                    {!readOnly && (
                      <div className="flex flex-col gap-3 mt-auto">
                        ...
                        <button
                          ...
                          className="group/btn relative w-full py-4 bg-slate-50 border border-slate-200 text-slate-900 font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl hover:bg-slate-100 transition-all active:scale-95"
                        >
              ...
              <div className="space-y-4">
                {sortedPoolApplicants.length > 0 ? sortedPoolApplicants.map(guard => (
                  <div
                    key={guard.id}
                    onClick={() => setDetailGuard(guard)}
                    className="group w-full text-left bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 flex items-center justify-between gap-6 cursor-pointer overflow-hidden relative"
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-black text-slate-900 uppercase tracking-tight leading-none group-hover:text-primary transition-colors">
                          {guard.full_name}
                          {((guard.education_history || []).some(e => (e.level || '').toLowerCase().includes('military') || (e.level || '').toLowerCase().includes('police') || (e.level || '').toLowerCase().includes('jkt') || e.weapon_proficiency === 'pass')) && (
                        ...
                        <div className="flex flex-wrap gap-2 text-xs text-slate-600 items-center">
                          ...
                          <span className="text-slate-400 bg-slate-50 px-2 rounded border border-slate-100">
                            Docs {[
          ...
          <div className="space-y-6">
            ...
            {interviewApplicants.map(guard => (
              <div
                key={guard.id}
                onClick={() => setDetailGuard(guard)}
                className="cursor-pointer text-left bg-white p-8 rounded-[3.5rem] border border-slate-200 shadow-sm flex flex-col md:flex-row gap-8 items-start w-full hover:shadow-xl transition-all focus:outline-none focus:ring-2 focus:ring-primary/50"
              >
                <span className="w-16 h-16 rounded-3xl border border-slate-100 overflow-hidden shrink-0 shadow-lg">
                  {guard.passport_photo_url
                    ...
                    : <div className="w-16 h-16 bg-slate-50 text-slate-400 rounded-3xl flex items-center justify-center font-black text-2xl">{guard.full_name?.[0] || 'G'}</div>}
                </span>
                <div className="flex-grow">
                  <div className="flex items-center gap-4 mb-2">
                    <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">{guard.full_name}</h3>
                    <span className="bg-emerald-600 text-white text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md shadow-lg shadow-emerald-500/20">Interview Ready</span>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-slate-400">
                    <div>Company: <strong className="text-slate-900">{companies.find(c => c.id === guard.company_id)?.name || '—'}</strong></div>
                    <div>Site: <strong className="text-slate-900">{sites.find(s => s.id === guard.current_site_id)?.name || '—'}</strong></div>
                    <div>Supervisor: <strong className="text-slate-900">{profiles.find(p => p.id === guard.assigned_supervisor_id)?.full_name || '—'}</strong></div>
                  </div>
