...
export const GuardWizard: React.FC<{ guards: Guard[], userRole: UserRole, initialData?: Partial<Guard>, onComplete: (guard: Guard, isApplicantFlow?: boolean) => void, isApplicantFlow?: boolean, isReadOnly?: boolean }> = ({ initialData, onComplete, isApplicantFlow = false, isReadOnly: readOnlyProp, userRole }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    let nextValue = type === 'checkbox' ? (e.target as HTMLInputElement).checked : value;
    let dobToUpdate = undefined;

    // Auto-extract DOB from NIDA
    if (name === 'nida_number') {
      const clean = String(nextValue).replace(/\D/g, '').slice(0, 20);
      nextValue = clean; // Enforce numeric only for NIDA
      if (clean.length >= 8) {
    ...
    setFormData(prev => {
      ...
      if (dobToUpdate) nextData.dob = dobToUpdate;
      return nextData;
    });
    ...
    const msg = fieldError(name, String(nextValue));
    setErrors(prev => {
      const next = { ...prev };
  ...
  const getAllErrors = (): ValidationErrors => {
    ...
    // Next of Kin Validation Analysis
    if (!formData.next_of_kin_name) {
      newErrors.next_of_kin_name = 'This field is required.';
      console.error("KIN_VALIDATION_FAILED: next_of_kin_name - This field is required.");
    }
    const kinPhErr = fieldError('next_of_kin_phone', formData.next_of_kin_phone);
    if (kinPhErr) {
      newErrors.next_of_kin_phone = kinPhErr;
      console.error(`KIN_VALIDATION_FAILED: next_of_kin_phone - ${kinPhErr} (Status: ${formData.next_of_kin_phone ? 'Invalid' : 'Missing'})`);
    }
    if (!formData.next_of_kin_relationship) {
      newErrors.next_of_kin_relationship = 'This field is required.';
    ...
    if (formData.guarantors.length < 2) {
      ...
      console.error(`GUARANTOR_VALIDATION_FAILED: count - Current: ${formData.guarantors.length}, Required: 2`);
    } else {
      formData.guarantors.forEach((g, idx) => {
        const checkField = (f: string, val: any, label: string) => {
          if (!val) {
            newErrors[`g_${f}_${idx}`] = 'This field is required.';
            console.error(`GUARANTOR_VALIDATION_FAILED [Index: ${idx}]: g_${f} - ${label} is required.`);
          }
        };

        if (!g.name) {
          newErrors[`g_name_${idx}`] = 'This field is required.';
          console.error(`GUARANTOR_VALIDATION_FAILED [Index: ${idx}]: g_name - Name is required.`);
        }
        if (!g.relationship) {
          newErrors[`g_rel_${idx}`] = 'This field is required.';
        ...
        if (!(g as any)?.id_copy_url) {
          newErrors[`g_id_copy_${idx}`] = 'This field is required.';
          console.error(`GUARANTOR_VALIDATION_FAILED [Index: ${idx}]: g_id_copy - ID Copy is missing.`);
        }
        if (!(g as any)?.residence_letter_url) {
          newErrors[`g_residence_letter_${idx}`] = 'This field is required.';
          console.error(`GUARANTOR_VALIDATION_FAILED [Index: ${idx}]: g_residence_letter - Residence Letter is missing.`);
        }

        if (!g.phone || !isTZPhone(g.phone)) {
          newErrors[`g_phone_${idx}`] = 'Please enter a valid Tanzanian phone number (e.g., 0755123456).';
          console.error(`GUARANTOR_VALIDATION_FAILED [Index: ${idx}]: g_phone - ${g.phone ? 'Format invalid' : 'Missing'}`);
        }
      });
    }
    return newErrors;
  ...
  const getStepErrors = (step: number): ValidationErrors => {
    ...
    if (step === 3) {
      // Education validation handled centrally; step check not required here
    }
    if (step === 4) {
  ...
  const handleSubmit = async () => {
    ...
    // 1. Enhanced Validation Map Logging
    console.warn("FULL_VALIDATION_MAP", {
      timestamp: new Date().toISOString(),
      errorCount: Object.keys(allErrors).length,
      errors: allErrors,
      formDataContext: {
        nida: formData.nida_number,
        full_name: formData.full_name,
        step: currentStep,
        guarantorCount: formData.guarantors.length,
        eduCount: formData.education_records.length
      }
    });
    ...
    // 2. Auth Context Audit
    const authToken = localStorage.getItem('amini_auth_token') || localStorage.getItem('token');
    console.warn("AUTH_CONTEXT_CHECK:", {
      tokenPresent: !!authToken,
      tokenLength: authToken?.length,
      // Attempt to peek at expiry if it's a JWT
      tokenType: authToken?.startsWith('ey') ? 'JWT' : 'Unknown',
      timestamp: new Date().toISOString()
    });
    ...
    try {
      ...
      console.error("SUBMISSION_ERROR_CAUGHT:", error);
  ...
  const addEducation = () => {
    setFormData(prev => ({
      ...
      education_records: [
        ...
        { id: `temp-${Date.now()}`, guard_id: '', level: 'secondary', institution_name: '', year: '', certificate_url: '' } as any
      ]
    }));
  };
      ...
      <div className="space-y-16">
        <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <InputField
              label="Date of Birth (Auto-populated from NIDA)"
              name="dob"
              type="date"
              value={formData.dob}
              onChange={handleChange}
              error={errors.dob}
              disabled={isReadOnly || (formData.nida_number.replace(/\D/g, '').length >= 8 && formData.nida_number.startsWith(formData.dob.replace(/-/g, '')))}
            />
          ...
          <div className="space-y-6">
            {formData.education_records.map((edu, index) => (
              <div key={index} className="p-6 bg-slate-50 rounded-2xl border border-slate-200 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <InputField label="Completion Year (YYYY)" name={`edu_year_${index}`} value={edu.year} onChange={(e) => updateEducation(index, 'year', e.target.value.replace(/[^0-9]/g, '').slice(0, 4))} placeholder="YYYY" disabled={isReadOnly} error={errors[`edu_year_${index}`]} />
                  <FileUploader label="Certificate Scan (PDF/JPG/PNG, max 5MB)" fileUrl={edu.certificate_url} onUpload={(url) => updateEducation(index, 'certificate_url', url)} onRemove={() => updateEducation(index, 'certificate_url', '')} className="h-28" disabled={isReadOnly} acceptTypes="image/jpeg,image/png,application/pdf" />
                </div>
              </div>
            ))}
