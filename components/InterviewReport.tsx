  ...
  const handleExportCsv = () => {
    try {
      ...
      const rows = logs.map(l => {
        ...
        let commentsObj: any = {};
        try {
          if (typeof l.comments === 'string') {
            commentsObj = JSON.parse(l.comments);
          } else if (typeof l.comments === 'object' && l.comments !== null) {
            commentsObj = l.comments;
          }
        } catch (e) { }
        ...
        const mappedOut = [
          ...
          (g?.full_name || ''),
          l.outcome,
          (l.rejection_reason || commentsObj?.rejection_reason || ''),
          (l.interview_date || ''),
          (l.interview_notes || commentsObj?.interview_notes || ''),
          (l.deployment_contract_url || ''),
          (l.created_at || '')
        ];

        return mappedOut.map(v => {
          const s = String(v ?? '');
                  ...
                  <tr key={log.id || `${log.guard_id}-${log.created_at}`}>
                    ...
                    <td className="px-8 py-6">
                      <p className="text-sm text-red-600 italic">"{(() => {
                        let c: any = {};
                        try { c = typeof log.comments === 'string' ? JSON.parse(log.comments) : (log.comments || {}); } catch { }
                        return log.rejection_reason || c?.rejection_reason || guard?.dossier_data?.rejection_reason || '—';
                      })()}"</p>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                      <p className="text-sm text-slate-700 truncate">{(() => {
                        let c: any = {};
                        try { c = typeof log.comments === 'string' ? JSON.parse(log.comments) : (log.comments || {}); } catch { }
                        return log.interview_notes || c?.interview_notes || '—';
                      })()}</p>
                    </td>
